...
### ESP 方框（植物 + 僵尸）
- **植物**：每株头顶一个方框（深色＝普通，红色＝当前选中），内容 `中文名  当前血量/上限`
- **僵尸**：每只头顶一个方框，**红色顶条＝敌人，青色顶条＝被魅惑的友军，黄色顶条＝当前选中**，
  内容 `中文名  血量/上限`（有护甲时额外显示 `+护甲值`）
...
│   ├─ ZombieDb.cs            僵尸中文名/数值读写/行为开关/变身
