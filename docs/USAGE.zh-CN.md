...
**标题栏上写着版本号**（当前 `v1.7.0（内置界面 · 8 页签）`）——
如果你看到的不是这个，说明游戏里跑的还是旧 DLL，重启游戏即可。

> **v1.7.0 新增**：融合页支持**批量融合 / 批量变身 / 批量升级**，并且可以设置
> **「融合后升级到 N 级」** —— 选定的同种植物一次全部融合并升级。
>
> **v1.6.0**：「诸神幸运」页 —— 改「诸神」系列（射击 / 诸神进化 / 无尽 / 炼狱）里的**幸运**。
>
> **v1.5.0**：僵尸也支持**多选批量修改**；战场上**左键按住拖拽 = 框选**；
> **游戏倍速改成"按一次应用一次"**（不再循环锁死）并可**自定义倍速**；
> 修掉了输入框"打一个 5 变成 55"的重复问题。

> **怎么确认自己看的是"内置菜单"而不是旧的外置窗口**：
> 内置菜单的标题栏有 `ESP: 开` / `隐藏 (F1)` / `×` 三个按钮，
> 下面是 **7 个页签**（功能开关 · 植物 · **僵尸** · 融合 · 沙盒 · 作弊动作 · 设置）。
...
| **鼠标左键按住拖拽**（战场上） | **框选**：框到的植物/僵尸一起进多选（详见下面「框选」） |
| 编辑数值时 **回车** / **ESC** | 确认 / 取消（退格删除；筛选框可以直接打字）|
| 标题栏 `隐藏(F1)` / `×` | 关掉菜单 |

> **输入框怎么打字**：点一下数值框就开始输入（蓝底黄框），
> **第一个字符会把原来的值整段替换掉**（以前是往后追加，所以"输入 5 会变成 55"），
> 想改原来的值就按退格进入普通编辑，回车确认、Esc 取消。
...
### 功能开关
全部 40+ 个开关，按 11 组折叠（**默认全部展开**）：通用 / 解锁与资源 / 关卡内 / 战斗 / 旅行词条 /
天赋 / 深渊抽奖券 / 经典作弊 / **速度·僵尸·工具** / 界面与 ESP / **诸神幸运**。
...
### 植物（支持多选批量修改）
- 左栏：**场上植物列表**，每行最左边有一个**勾选框**：
  - **点勾选框** = 把它加入「批量修改」（行变蓝、左侧蓝边，可勾多株）
  - **点行的其它位置** = 单选它（行变绿），编辑器显示它的数值
  - 下方四个按钮：`全选 / 反选 / 清空多选 / 取消单选（多选保留）`
- 右栏：**该植物的全部实时数值**，两列排布：
...
- 底部按钮：还原（已勾选的）植物的全部修改 / 刷新融合配方 / 去融合页

> **批量修改**：只要勾了至少一株，右栏顶部会出现蓝色横幅
> `★ 批量修改模式：已勾选 N 株 —— 下面的改动会一次应用到这 N 株`，
> 之后**每次数值修改、每个机制勾选、每次还原**都会一次作用到所有勾选的植物。
> 一株都没勾时，就只改单选的那一株（和以前一样）。
>
> 勾选按**对象指针**记录，列表每 0.25 秒重建也不丢；植物死掉或被融合替换后，
> 它的勾选会在下一帧自动清掉，不会虚高。
>
> 实测（`ACTION|Mark` 勾选 + `ACTION|BSet` 批量应用）：
>
> ```
> 不勾选，只选中第1株 -> 应用到 1 株   结果: 4000 / 300  / 300
> 勾选第1、3株        -> 应用到 2 株   结果: 7777 / 300  / 7777
> 清空多选，选中第2株 -> 应用到 1 株   结果: 7777 / 1234 / 7777
> ```

### 框选（左键拖拽多选，v1.5.0）
在**战场上**按住鼠标左键拖出一个框，松手后**框到的植物和僵尸会一起进多选**：

- 只有拖动超过 **8 像素**才算框选 —— 普通点击（种植物、收阳光）完全不受影响；
- 框选期间游戏的鼠标会被临时屏蔽（`Patch_Mouse_Update`），所以**不会一边拖一边种植物**；
- **只加不减**：手抖框到一片不会清掉已有的勾选，想清空用「清空多选」按钮；
- 框里的盒子会实时画出来（半透明绿底 + 绿边 + 尺寸提示），
  框选结果写进状态栏和日志（`[框选] 框到 植物 N 株 / 僵尸 M 只 …`）；
- 被框中的植物/僵尸，其 **ESP 方框会变成蓝色**，一眼就能看出哪些进了多选；
- 开关在「设置」页：`左键拖拽框选`（配置项 `BoxSelect`，**默认开**；这是界面辅助，不是作弊项）。
  关掉之后游戏鼠标完全恢复原样。

> **在菜单面板上拖 = 拖菜单**，不会触发框选（只在战场上生效）。
> 实测（真鼠标事件，`_mod\input_test.ps1`）：
> ```
> [框选] 已开始框选（左键拖拽即多选植物/僵尸）
> [框选] 框选：框到 植物 2 株 / 僵尸 0 只，新加入 植物 +2 / 僵尸 +0　→　多选现在共 植物 2 株 / 僵尸 0 只
> ```
...
### 僵尸（支持多选批量修改，v1.5.0）
和「植物」页一模一样的套路，只是对象换成僵尸（数据来自 `Board.zombieArray` + `Lawnf.GetAllZombies` + `FindObjectsOfType`）。

- 左栏：**场上僵尸列表**（`中文名 #编号  血量/上限  行N`），**被魅惑的会标 ★ 并用青色左边条**
  - 每行最左边有**勾选框**：点它加入批量修改（行变蓝）；点行其它位置 = 单选
  - 下方按钮：`全选 / 反选 / 清空多选 / 取消单选（多选保留）`
- 右栏：**该僵尸的全部实时数值**（两列，14 项）：
  当前血量 / 血量上限（都是 `long`）/ 移动速度 / 原始速度 / 攻击力 / 等级 / 所在行 /
  护甲一当前 / 护甲一上限 / 护甲二当前 / 护甲值 / 受伤倍率 / 冻结等级 / 中毒等级
- 行为开关 4 个：**无敌（持续回满血）/ 停止移动 / 持续冻结 / 魅惑（变友军）**
- 底部按钮：**秒杀（勾选的）僵尸 / 还原（勾选的）全部修改 / 换成别的僵尸…**（变身只支持单只）

> **僵尸批量修改**：勾了至少一只，右栏顶部就出现蓝色横幅
> `★ 批量修改模式：已勾选 N 只僵尸`，之后每个数值、每个行为开关、秒杀/还原
> 都会一次作用到全部勾选目标（和植物页逻辑完全一致）。
> 实测（`ACTION|ZMarkAll` + `ACTION|ZBSet`）：
> ```
> ZBSet 血量上限 = 5000  -> 4 只僵尸 maxhp 全部 = 5000
> ZFlag 持续冻结 = 1     -> 4 只僵尸 冻结等级 全部 = 3
> ```
>
> **冻结踩的坑**：这套构建里 `Zombie.SetFreeze(30f, 3)` 是**空操作**
> （不报错、但 `freezeLevel` 一直是 0），必须**直接写 `freezeLevel` 属性**才真的冻住。
> 关掉"持续冻结"时会主动解冻，否则它会一直冻着。

> **僵尸变身不能只改 `theZombieType`** —— 僵尸没有 `ReplaceSprite`，只改类型会出现"贴图没变"的老问题。
> 所以走的是"**在同一行同一位置新建一只目标类型，然后让原来那只 `Die`**"，
> 新僵尸由游戏完整初始化（贴图/血量/技能都对）。
>
> **魅惑**实测 `SetMindControl(0/1/2)` 三个等级都会把 `isMindControlled` 置为 True。

实测（`ACTION|ZSet` 逐项改完再读快照）：

```
改前: #0 僵尸 hp=270/270     行2 速=1   魅=0
改后: #0 僵尸 hp=8888/9999   行2 速=0.5 魅=1   选中=1
```

### 融合（支持批量，v1.7.0）
- 顶部显示选中植物的**分类**（基础植物 / 超级植物 / 终极植物 / 二次融合）与**合成来源**
  （"魅惑菇 + 西瓜投手"这种中文写法）
- 顶部右上角：**融合后升级**（点一下输入等级，`0` = 融合后不动等级）
- **配方表**：`伙伴植物 (#id) → 当前融合结果 (#id)`，全中文名，可滚动
  - **「直接融合」**：让选中的这株和该伙伴**当场融合**（真的换成融合体，模型/血量/子弹都是游戏重建的）
  - 多选（勾选/框选）了植物时，按钮变成 **「批量直接融合 ×N」**，一次作用到全部目标
  - **「改结果…」**：弹出中文植物选择器，改掉融合表里这一条配方（伙伴 → 你要的结果）
- 底部「自己加一条 / 换一条配方」：选伙伴、选结果 →「写入配方」
- **「直接变成「结果」那种」**：不查融合表，直接把选中的植物换成指定植物（多选时是**批量变身 ×N**）

#### 批量融合 / 批量升级（v1.7.0）
「植物」页勾选（或战场上左键拖框选中）**若干株**之后，融合页顶部会出现蓝色横幅：

```
★ 批量模式：目标 = 5 株（全部是 豌豆射手） —— 下面每个「直接融合」都会一次作用到它们全部（逐株处理）
```

- **同一种植物**时横幅会写明"全部是 XX"；混选也会列出各类型数量（融合按各自配方算，伙伴是同一个）
- 「直接融合」变成 **「批量直接融合 ×N」**，「用伙伴直接融合」变成 **「用伙伴批量融合 ×N」**
- **「批量升级到 N 级」**：把当前所有目标一次升到 N 级（N 取自"融合后升级"那个输入框）
- **「融合后升级」**：融合 / 变身出来的结果植物**直接设成该等级**（这样"融合 + 升级"一步到位，
  否则融合后对象换了新的指针，多选会失效）
- 批量是**逐株排队**做的（每株都要"先 Die 腾格子 → 游戏新建"），状态栏会显示 `（还剩 N 株）`，
  最后给一句汇总：`批量操作完成：成功 5 株 / 失败 0 株（共 5 株）`

实测（`ACTION|FuseAllUp|24|10`，5 株豌豆射手全部勾选）：

```
融合后: [('豌豆南瓜', 1318, 'lv=10', 'hp=4000/4000')] ×5      ← 5 株全部换成了真融合体，并且都升到 10 级
[融合] 融合成功：豌豆射手 #0 + 南瓜 → 豌豆南瓜 #1318，并升到 10 级   方式:腾空格子后由游戏新建（模型/属性完整）
[融合] 批量操作完成：成功 5 株 / 失败 0 株（共 5 株）。
```

`ACTION|UpgradeSel|7` → 3 株全部 `lv=7`；`ACTION|TransAll|24|3` → 3 株豌豆射手全部变成
`南瓜 #24` 且 `lv=3`（变身走的也是同一套"腾格子重建"流程）。
...
**游戏倍速（v1.5.0 改成"按一次应用一次"）**：

- 一排档位按钮（直接用游戏自己的 `GameSpeedMgr.Gears`：0.1 / 0.2 / 0.5 / 0.75 / 1 / 1.5 / 2 / 3）
- **自定义输入框**：点一下输入任意倍速（0.05 ~ 20，例：`2.5` / `0.75`），回车即应用
- `应用一次` / `恢复 1x` / `诊断` 三个按钮，右边实时显示 `当前实际: x.xx`

> **为什么改成一次性的**：以前是在主循环里**每 0.25 秒写一遍** `Time.timeScale`，
> 结果用户点一次 3x 之后就**再也改不回来**了 —— 游戏自己的倍速滑条、暂停菜单全被按回去。
> 现在只有你点按钮/回车那一刻应用一次（`RequestGameSpeed` → 主循环消费一次）
> 之后游戏想怎么改速度都随它，插件不再插手。日志每应用一次写一行：
> `[速度] 已应用一次：2.5x（不再常驻锁定，之后游戏自己也能改）`。

### 设置
菜单字号（10~26）、**菜单窗口复位到左上角**、ESP 显示项（方框/名称/血量/序号/加粗/字号）、
**左键拖拽框选开关**、**植物读数诊断**（采用来源 + 5 条路径各自命中数 + 游戏侧计数）、热键说明。

### 诸神幸运（v1.6.0）
「诸神」系列（诸神射击 / 诸神进化 / 无尽 / 炼狱）里的**幸运**值修改。

- 顶部信息带（**在诸神模式里是绿框、不在是红框**）：模式 + 轮次 + 难度 + 词条分，
  当前幸运 / 上限，以及游戏自己那行幸运说明（`ShootingManager.LuckyString`）
- **不在诸神模式**时，信息带右边给两个快捷入口：`进诸神(140)` / `进炼狱(194)`
- `幸运` 输入框 + `上限` 输入框 + `应用一次` / `记下原值` / `还原原值` / `诊断`
- **快速设置**：50 / 100 / 150 / 200 / 300 / 500 / 800 / 1000 / 5000 / 9999（点一下立刻应用一次）
- **幸运上限**：300 / 500 / 1000 / 3000 / 9999 / `上限 ×2` / `上限设为当前幸运`
- **常驻锁定**勾选框（默认关）：打开后每 0.25 秒把幸运拉回输入框里的值
- 底部说明块写明机制、关卡号、以及"宝箱挑战的幸运等级不碰"

> **游戏侧的位置**（IL2CPP 实证）：`GameLevel.RogueShooting.ShootingManager`
> · `public static ShootingManager Instance;`（**只有诸神类关卡才存在**）
> · `public float maxLucky;`（上限）· `private float _lucky;` · `public float Lucky { get; set; }`
> · 还有保底相关：`pityThreshold` / `pityEnabled` / `totalPityTriggered` / `noDiamondCount`（本页只读展示）
>
> **改的时候必须先把上限抬上去**：`ShootingManager.Lucky` 的 setter 会按 `maxLucky` 钳制，
> 直接写 300 而上限只有 2.5 是没用的。插件会 `if (maxLucky < 目标) maxLucky = 目标;` 再写幸运，
> 写完还会读回来核对，被钳制就再去写一次私有字段 `_lucky`，并在日志里写明。
>
> **诸神对应的关卡号**（挑战模式，实测）：**140 = RogueShooting（诸神进化）**、
> 194 = RogueShooting_hellMode（炼狱）、173/174 = GodShooting、149 = EndlessShooting、
> 113/121/130/159/170 = Shooting4~8、145 = PlayerShooting —— 它们共用同一个 `ShootingManager`。
> 普通关卡/冒险/旅行/深渊都读不到它（`Instance` 为空）。

实测（`ACTION|Luck|…`，读 `STATE` 里的 `luck` 对象）：

```
进诸神前            : luck.on=0  「当前不在诸神类模式」
进挑战 140（诸神）  : luck {'on':1,'lucky':0,   'max':2.5, 'pity':50,'pityOn':1,'str':'0'}
ACTION|Luck|300     : luck {'on':1,'lucky':300, 'max':300,  'str':'30000'}     ← 上限自动跟上
ACTION|Luck|9999    : luck {'on':1,'lucky':9999,'max':9999}
ACTION|LuckMax|1000 : luck {'on':1,'lucky':9999,'max':1000}
ACTION|Luck|750     : luck {'on':1,'lucky':750, 'max':1000}
LuckLock=1 + Luck|123 -> 连续 3 秒读回都是 123（锁定生效）
真鼠标点 UI         : 点「300」预设 -> lucky 300 / 上限 300；点「上限 9999」-> max 9999
```

> **宝箱挑战（`ChestChallenge`，0~30 的幸运等级）没做**：它是纯数据对象
> （`ChallengeLevelData` 的子类，不是 MonoBehaviour），游戏里没有"当前关卡数据"的静态入口，
> 拿不到实例就没法安全修改。诸神这套（`ShootingManager`）是静态 `Instance`，所以能做。

### ESP 方框（植物 + 僵尸）
- **植物**：每株头顶一个方框（深色＝普通，红色＝当前选中，**蓝色＝已进多选**），内容 `中文名  当前血量/上限`
- **僵尸**：每只头顶一个方框，**红色顶条＝敌人，青色顶条＝被魅惑的友军，黄色顶条＝当前选中，
  蓝色顶条＝已进多选**，内容 `中文名  血量/上限`（有护甲时额外显示 `+护甲值`）

**左键点方框 = 选中它**，然后就能在「植物」页 /「僵尸」页里改。
**在战场上左键拖拽 = 框选**（见上面「框选」一节）。

僵尸 ESP 的开关在「设置」页，也可以直接改配置：`ZombieEsp`（总开关）/ `ZombieEspName` /
`ZombieEspHp` / `ZombieEspHeight`（方框相对精灵的高度，僵尸比植物高，默认 1.1）。
...
| `BoxSelect` | `true` | 左键拖拽框选（**界面辅助，不是作弊项**） |
| `LuckyValue` / `LuckyMaxValue` / `LuckyLock` | `0` / `0` / `false` | 诸神幸运：目标值 / 上限 / 常驻锁定（**默认不改**） |
| `FuseUpgradeLevel` | `0` | 融合 / 变身之后把结果升到几级（**0 = 不升**） |
...
| **融合** | **批量融合 / 批量变身 / 融合后升级** | 同一套两阶段流程**排成队列**逐株执行（`Actions.RequestFuseBatch` / `RequestTransmuteBatch`） |
...
| 输入 | **左键拖拽框选**（正在框选时也屏蔽游戏鼠标） | 前缀 `Mouse.Update` 返回 `!框选中` |
| 输入 | **输入框消重**（同一个键被派发两次 → 只收一次） | IMGUI `Event` 层过滤（见 6.14） |
| **速度** | **游戏倍速（0.05x~20x，按一次应用一次）** | 写 `Time.timeScale` + `GameAPP.config.gameSpeed`（档位用游戏自己的 `GameSpeedMgr.Gears`：0.1~3） |
| **选择** | **框选植物/僵尸进多选** | `EspOverlay` 里的屏幕矩形与框求交（`BoxSelect` + `MenuUI.MarqueeSelect`） |
| **诸神** | **改幸运（值 / 上限 / 常驻锁定 / 还原）** | 直接写 `GameLevel.RogueShooting.ShootingManager.Instance` 的 `maxLucky` + `Lucky`（`LuckDb`） |
| **僵尸** | **单只/批量僵尸修改（界面）** | 周期写 `Zombie` 字段（`ZombieDb` + `ZombieOverrides`） |
...
| `ACTION\|Tools` | 打印 `Board.freeCD` / `GetGloveCD()` 和所有 `InGameTool` 的冷却值 |
| `ACTION\|ZSel\|<指针>` / `ZInfo` | 选中僵尸 / 打印它的全部字段与行为开关 |
| `ACTION\|ZSet\|<字段下标>\|<值>` | 按下标改选中僵尸的字段 |
| `ACTION\|ZFreeze` / `ZKill` / `ZMind[\|<等级>]` | 冻结 / 秒杀 / 魅惑选中的僵尸 |
| `ACTION\|ZMarkAll` / `ZMark` / `ZMarkClear` / `ZMarkCount` | 僵尸多选：全选 / 勾选选中的那只 / 清空 / 查询数量 |
| `ACTION\|ZBSet\|<字段下标>\|<值>` | **僵尸批量修改**（作用到全部勾选的僵尸，没勾选就作用到单选的） |
| `ACTION\|ZFlag\|<开关下标>\|1或0` | 僵尸行为开关批量设置（0无敌 1停移 2冻结 3魅惑） |
| `ACTION\|BoxSel\|<x>\|<y>\|<w>\|<h>` | **框选自检**：直接喂一个屏幕矩形走一遍框选逻辑 |
| `ACTION\|BoxSelect\|1或0` | 开关左键拖拽框选 |
| `ACTION\|Luck\|<值>` | 诸神幸运：把幸运设成该值（上限自动跟上），主循环里应用一次 |
| `ACTION\|LuckMax\|<值>` | 诸神幸运：设置幸运上限 |
| `ACTION\|LuckLock\|1或0` | 诸神幸运：常驻锁定开关 |
| `ACTION\|LuckInfo` / `LuckRemember` / `LuckRestore` | 打印幸运诊断 / 记下原值 / 还原原值 |
| `ACTION\|ScanLevels\|<LevelType>` | 打印该类型全部关卡（编号:名称 类名），用来找关号（日志 `[关卡表]`） |
| `ACTION\|CurLevel` | 打印当前关卡的 LevelData（日志 `[当前关]`） |
| `ACTION\|Fuse\|<伙伴类型>` | 让当前选中的植物与该伙伴直接融合 |
| `ACTION\|FuseAll\|<伙伴类型>` | **批量融合**：多选（勾选/框选）的植物全部与该伙伴融合一次 |
| `ACTION\|FuseAllUp\|<伙伴类型>\|<等级>` | 批量融合 + 融合后把结果升到该等级 |
| `ACTION\|UpgradeSel\|<等级>` | 把当前多选/单选的植物一次升到该等级 |
| `ACTION\|TransAll\|<类型>\|<等级>` | 批量"直接变成"该类型（同一套腾格子重建流程） |
| `ACTION\|FusionTest\|<类型>` | 跑一遍融合页的全部游戏调用并输出报告（不需要看屏幕就能验证） |

> `ACTION` 类指令**不回包**（避免与 `STATE` 回复错位），发完即可。
> `STATE` 里新增了 `mark` / `zmark`（多选数量），植物/僵尸条目里新增 `mk`（是否已进多选），
> 以及 `luck`（诸神幸运对象：`on/lucky/max/stage/difficulty/endless/hell/pity/str`）。

> **真鼠标事件自检**：`_mod\input_test.ps1` 用 Win32 `SetCursorPos` + `mouse_event` + `keybd_event`
> 真点真按，用来验证"点菜单、打字、框选"这些只靠 IPC 验证不了的路径。
> 注意本机是 **125% DPI**、游戏客户区在屏幕 `(2, 39)` 偏移，所以脚本里把游戏坐标换算成
> `(game + (2,39)) / 1.25` 再交给系统；换机器要重新标定（对着截图量面板边框即可）。
...
> **v1.5.0 修正**：以前是**每 0.25 秒写一遍**（"常驻保持"），结果是"点一次就永久锁死"，
> 用户明确反馈过"游戏速度应该是选定后按一次就应用一次而不是循环锁死"。
> 现在只在**点按钮 / 回车 / IPC 请求**时应用一次：
> `RequestGameSpeed(v)`（登记）→ `RunPendingSpeed()`（主循环里消费一次，应用完清空）。
> 想改回 1x 就点「恢复 1x」，游戏自己的倍速滑条也照样能用。

实测：`ACTION|Speed|2.5` → 日志恰好一行 `[速度] 已应用一次：2.5x`，
之后 3 秒内不再出现任何速度日志（说明没有循环写）；界面自定义输入 `5` → `Time.timeScale=5`。

### 6.14 输入框"打一个 5 变成 55"：同一个按键被派发两次（v1.5.0 修复）

用户反馈"输入时会重复，比如输入 5 会变成 55"。往 `EditKey` 里加了一行调试日志之后真相很清楚：

```
[按键调试] KeyDown keyCode=Alpha5 char=0   (' ') frame=4358     ← keyCode 通道
[按键调试] KeyDown keyCode=None   char=53  ('5') frame=4358     ← 字符通道
```

**按一次数字键，IMGUI 在这套构建里会派发两个 `KeyDown` 事件**（同一帧）：
一个只带 `keyCode`，一个只带 `character`。两个都追加 → "55"。
（函数键如 F2 没有字符通道，所以热键一直是正常的，只有输入框会中招。）

修法（`MenuUI.AppendChar`）：同一个字符、来源不同、同一帧（或 0.12 秒内）出现第二次 → 认作回声丢掉，
丢完把记录清空，所以**连按同一个键（9999）不受影响**。日志会写：
`[输入] 已忽略重复的 '5'（同一按键被派发两次：keyCode 通道 + 字符通道）`。

顺带修了**预填语义**：点开数值框会显示当前值，以前直接在后面追加（输入 5 → 55），
现在**第一个字符整段替换**（`_editFresh`），按退格则切回普通编辑。

实测（真键盘事件 `keybd_event`）：点自定义倍速框 → 输入一个 `5` → 回车 →
`[速度] 已应用一次：5x`（修复前是 `20x`，因为 "55" 被上限截断）。
...
`BoardSpawner_SummonZombies`、`Lawnf_GetGloveCD`、`CreatePlant_SetPlant`）。
...
v1.5.0 的实测（`_mod\probe_v150.py`，同时用了 IPC 和真鼠标/真键盘）：

```
僵尸多选批量     ACTION|ZMarkAll + ZBSet|1|5000  ->  4 只僵尸 maxhp 全部 = 5000
僵尸行为开关     ACTION|ZFlag|2|1               ->  4 只僵尸 冻结等级 全部 = 3
僵尸单选改速度   ACTION|ZSet|2|0.25             ->  spd = 0.25
框选(IPC 矩形)   ACTION|BoxSel|0|0|3000|3000    ->  mark 9 / zmark 4（全部框到）
框选(小矩形)     ACTION|BoxSel|0|0|3|3          ->  mark 0 / zmark 0
框选(真鼠标拖拽) input_test.ps1 -Mode drag      ->  [框选] 框到 植物 2 株 … 新加入 植物 +2
倍速一次性       ACTION|Speed|2.5                ->  日志只出现一行「已应用一次：2.5x」
倍速自定义输入   真键盘输入 "5" + 回车            ->  [速度] 已应用一次：5x（不是 55/20x）
菜单输入消重     同上                            ->  [输入] 已忽略重复的 '5'（keyCode 通道 + 字符通道）
```
...
4. **单株覆盖按对象指针索引**。植物销毁后若新植物分配到同一地址，旧覆盖会套到新植物。
   融合时已经把数值转移过去了，跨关卡建议按一下「清空全部植物修改记录」。
5. **`Wheel`（转盘类工具）没有静态 `Instance`**，只能靠遍历所有 `InGameTool` 兜底；
   如果它在某个界面下没被 `FindObjectsOfType` 找到，那个工具的无冷却就不生效。
6. **卡片/工具无冷却的"还原"依赖按指针的缓存**。如果先开作弊再切关卡（对象全部重建），
   缓存超过 600 条会整体清空，旧的 `fullCD` 就找不回来 —— 不过下一关卡片本来就是新建的，
   值也是对的，实际影响很小。
7. **`Board.freeCD` 会被我们按需改写**。虽然是"开时记原值、关时还原"，
   但如果游戏在同一段时间里自己也在改这个字段，还原可能覆盖游戏原本的意图（下一关会自然恢复）。
8. **虚方法重写**：`Plant.TakeDamage`/`Zombie.TakeDamage` 是虚方法，补的是基类实现；
   子类若重写则那条路径拦不住。植物无敌额外加了"周期回满血"兜底。
9. **没做动态调试**：本插件是"按签名打补丁 + 运行时日志验证"的产物，没有下断点逐帧跟踪。
   功能无效时把 `BepInEx\LogOutput.log` 里 `[FAIL]`/`[融合]`/`[卡片]`/`[工具]` 开头的行发我。
10. **框选只在战场上生效**：起点落在菜单面板上就是"拖菜单"，不会框选（这是故意的）。
    而且框选**只加不减**（手抖框一片不会清掉已有勾选），清空要按「清空多选」。
11. **游戏倍速不再"常驻保持"**：这是按用户要求改的（点一次应用一次）。
    如果你之后用游戏自己的倍速滑条改了速度，插件不会把它拉回来 —— 想再用插件的值就再点一下。
12. **僵尸「换成别的僵尸」只支持单只**（变身是"新建 + 让原来那只 Die"，批量做要多阶段排队）；
    批量数值/行为开关/秒杀/还原都支持多选。
13. **冻结依赖直接写 `freezeLevel` 属性**（游戏的 `SetFreeze` 在这套构建里是空操作）。
    如果游戏更新后字段改名，这个功能会静默失效（`ZombieDb.Freeze` 里两个都做了，日志可查）。
14. **框选/多选按对象指针记录**：植物或僵尸消失后不会自动从集合里立刻删掉，
    但每次读取目标列表时会把"已经不在场上的指针"清理掉，所以不会越来越虚高。
15. **诸神幸运只在诸神类关卡有效**：`ShootingManager.Instance` 只在诸神系列里存在，
    普通关卡点按钮会提示"当前不在诸神模式"。另外**宝箱挑战（0~30 的幸运等级）不做** ——
    它是纯数据对象（`ChallengeLevelData` 子类，不是 MonoBehaviour），
    游戏里没有"当前关卡数据"的静态入口，拿不到实例就没法安全修改。
16. **幸运的"常驻锁定"和倍速不一样**：倍速是"点一次应用一次"（按用户要求），
    但幸运额外提供了一个**默认关闭**的常驻锁定，因为游戏可能会消耗/改写幸运；
    不想被持续改写就别打开它（打开后日志/状态栏都有提示）。
...
│   ├─ ModConfig.cs           全部配置项
│   ├─ Patches.cs             公共逻辑 + 18 个补丁类 + 植物/僵尸列表/选中/融合状态机/倍速请求
│   ├─ Overrides.cs           单株植物覆盖 + 子弹覆盖 + 融合后数值转移
│   ├─ PlantDb.cs             植物中文名缓存/类型表/数值读写/融合配方/两阶段融合
│   ├─ ZombieDb.cs            僵尸中文名/数值读写/行为开关/冻结/变身
│   ├─ ZombieOverrides.cs     每只僵尸的覆盖与应用
│   ├─ LuckDb.cs              诸神幸运（读/写/上限/锁定/原值还原/只读缓存）
│   ├─ BoxSelect.cs           左键拖拽框选（屏幕矩形求交 → 进多选）
│   ├─ UiSkin.cs              只用确认可用的 IMGUI 接口手绘控件
│   ├─ MenuUI.cs              游戏内菜单全部界面（含 MenuOverlay 外壳 + 输入消重）
│   └─ EspOverlay.cs          ESP 方框（植物 + 僵尸 + 框选矩形）
...
├─ probe_v150.py              v1.5.0 新功能自检（多选/框选/倍速/默认全关）
├─ input_test.ps1             真鼠标/真键盘自检（点菜单、打字、拖拽框选）
├─ install_when_closed.ps1    等游戏退出后自动安装
└─ uninstall.ps1              一键回滚
```

> `input_test.ps1` 里的坐标换算（`(game + (2,39)) / 1.25`）是**本机 125% DPI** 标定出来的，
> 换机器要么重新标定，要么只用 IPC 那套（`probe_v150.py`）验证。
> 这两个脚本是**本地自检工具**（路径写死了本机游戏目录），没有随仓库发布。
